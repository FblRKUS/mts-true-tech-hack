...
GPTHUB_API_URL = os.environ.get('GPTHUB_API_URL', '').strip()
if not GPTHUB_API_URL:
    # Backward compatibility: if not explicitly set, reuse primary OpenAI-compatible upstream.
    fallback_openai_url = (
        OPENAI_API_BASE_URLS.value[0] if isinstance(OPENAI_API_BASE_URLS.value, list) and OPENAI_API_BASE_URLS.value else ''
    )
    GPTHUB_API_URL = str(fallback_openai_url or '').strip()

if GPTHUB_API_URL.endswith('/'):
    GPTHUB_API_URL = GPTHUB_API_URL[:-1]
if GPTHUB_API_URL.endswith('/v1'):
    GPTHUB_API_URL = GPTHUB_API_URL[:-3]

GPTHUB_API_URL = PersistentConfig(
    'GPTHUB_API_URL',
    'gpthub.api_url',
    GPTHUB_API_URL,
)

OPENAI_API_TITLE = os.environ.get('OPENAI_API_TITLE', '').strip()
GPTHUB_AUTO_BOOTSTRAP_OPENAI = (
    os.environ.get('GPTHUB_AUTO_BOOTSTRAP_OPENAI', 'true').strip().lower() == 'true'
)
...
openai_base_urls = list(OPENAI_API_BASE_URLS.value) if isinstance(OPENAI_API_BASE_URLS.value, list) else []
openai_keys = list(OPENAI_API_KEYS.value) if isinstance(OPENAI_API_KEYS.value, list) else []
openai_configs = OPENAI_API_CONFIGS.value if isinstance(OPENAI_API_CONFIGS.value, dict) else {}
openai_api_base_url_env = os.environ.get('OPENAI_API_BASE_URL', '').strip()
openai_api_base_urls_env = os.environ.get('OPENAI_API_BASE_URLS', '').strip()
openai_api_title_env = os.environ.get('OPENAI_API_TITLE', '').strip()
openai_api_key_env = os.environ.get('OPENAI_API_KEY', '').strip()
openai_api_keys_env = os.environ.get('OPENAI_API_KEYS', '').strip()
explicit_openai_env = any(
    [
        openai_api_base_url_env,
        openai_api_base_urls_env,
        openai_api_title_env,
        openai_api_key_env,
        openai_api_keys_env,
    ]
)

# Keep user-defined title for the primary connection, when provided via env.
if OPENAI_API_TITLE:
    primary_config = openai_configs.get('0', {})
    if not isinstance(primary_config, dict):
        primary_config = {}
    if not str(primary_config.get('title', '')).strip():
        openai_configs = {**openai_configs, '0': {**primary_config, 'title': OPENAI_API_TITLE}}

# Auto-bootstrap GPTHub as an additional OpenAI-compatible connection.
if GPTHUB_AUTO_BOOTSTRAP_OPENAI and GPTHUB_API_URL.value:
    gpthub_openai_url = _normalize_openai_base_url(f'{GPTHUB_API_URL.value}/v1')
    matched_idx = None
    for idx, url in enumerate(openai_base_urls):
        if _normalize_openai_base_url(str(url)) == gpthub_openai_url:
            matched_idx = idx
            break

    replace_default_placeholder = (
        not explicit_openai_env
        and matched_idx is None
        and len(openai_base_urls) == 1
        and _normalize_openai_base_url(str(openai_base_urls[0])) == 'https://api.openai.com/v1'
    )

    if replace_default_placeholder:
        openai_base_urls[0] = gpthub_openai_url
        ...
        openai_base_urls.append(gpthub_openai_url)
        openai_keys.append('')
        matched_idx = len(openai_base_urls) - 1

    gpthub_config = openai_configs.get(str(matched_idx), {})
    if not isinstance(gpthub_config, dict):
        gpthub_config = {}
    gpthub_key = str(openai_keys[matched_idx] if matched_idx is not None and matched_idx < len(openai_keys) else '').strip()
    next_gpthub_config = {**gpthub_config}

    if not str(next_gpthub_config.get('title', '')).strip():
        next_gpthub_config['title'] = 'GPTHub'

    # If GPTHub connection has no key, explicitly mark auth as "none"
    # so UI dropdown does not fallback to "Bearer".
    if gpthub_key == '':
        current_auth_type = str(next_gpthub_config.get('auth_type', '')).strip().lower()
        if current_auth_type in ('', 'bearer'):
            next_gpthub_config['auth_type'] = 'none'

    openai_configs = {**openai_configs, str(matched_idx): next_gpthub_config}

# Ensure key array length remains aligned with URL array.
if len(openai_keys) < len(openai_base_urls):
    openai_keys += [''] * (len(openai_base_urls) - len(openai_keys))
elif len(openai_keys) > len(openai_base_urls):
    openai_keys = openai_keys[: len(openai_base_urls)]

OPENAI_API_BASE_URLS.value = openai_base_urls
OPENAI_API_KEYS.value = openai_keys
OPENAI_API_CONFIGS.value = openai_configs
