...
def get_source_context(sources: list, source_ids: dict = None, include_content: bool = True) -> str:
    ...
    for source in sources:
        for doc, meta in zip(source.get('document', []), source.get('metadata', [])):
            ...
            body = str(doc or '') if include_content else ''

            if include_content and len(body) > max_doc_chars:
            ...
            if len(context_string) + len(source_chunk) > max_total_chars:
                remaining = max_total_chars - len(context_string)
                if remaining > 0:
                    context_string += source_chunk[:remaining]
                    truncated = True
                if truncated:
                    log.warning(
                        'RAG source context was truncated to keep request size bounded '
                        f'(max_total_chars={max_total_chars}, max_doc_chars={max_doc_chars}).'
                    )
                return context_string

            context_string += source_chunk
...
async def convert_url_images_to_base64(form_data):
    ...
    for message in messages:
        ...
        for item in content:
            ...
            preserved_metadata = {}
            for key in ('source_ref', 'mime_type', 'size_bytes', 'width', 'height'):
                if item.get(key) is not None:
                    preserved_metadata[key] = item.get(key)
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if model.get('owned_by') == 'arena':
        def is_arena_text_candidate(available_model: dict) -> bool:
            if available_model.get('owned_by') == 'arena':
                return False
            model_id = str(available_model.get('id') or '').lower()
            if model_id in {'autoselect', ''}:
                return False
            if any(
                token in model_id
                for token in ('image', 'embedding', 'whisper', 'tts', 'stt', 'speech', 'rerank', 'bge-')
            ):
                return False
            meta = (available_model.get('info') or {}).get('meta') or {}
            capabilities = meta.get('capabilities') or {}
            if capabilities.get('completion') is False:
                return False
            if capabilities.get('image_generation') is True:
                return False
            modality = str(meta.get('modality') or '').lower()
            if modality in {'image', 'audio', 'embedding'}:
                return False
            return True
        ...
        if arena_model_ids and arena_filter_mode == 'exclude':
            arena_model_ids = [
                ...
                if is_arena_text_candidate(available_model) and available_model['id'] not in arena_model_ids
            ]
        ...
        if isinstance(arena_model_ids, list) and arena_model_ids:
            arena_model_ids = [
                ...
                if is_arena_text_candidate(request.app.state.MODELS.get(model_id, {}))
            ]
            selected_model_id = random.choice(arena_model_ids) if arena_model_ids else None
        else:
            arena_model_ids = [
                available_model['id']
                for available_model in request.app.state.MODELS.values()
                if is_arena_text_candidate(available_model)
            ]
            selected_model_id = random.choice(arena_model_ids) if arena_model_ids else None

        selected_model = request.app.state.MODELS.get(selected_model_id) if selected_model_id else None
        if selected_model:
            log.info(
                "arena model resolved",
                extra={"selected_model_id": selected_model_id, "candidate_count": len(arena_model_ids)},
            )
            model = selected_model
            form_data['model'] = selected_model_id
            metadata['selected_model_id'] = selected_model_id
        else:
            log.warning("arena model resolution failed: no eligible chat models")
    ...
    if chat_id and parent_message_id and not chat_id.startswith('local:'):
        ...
        if db_messages:
            ...
            def _gallery_source_ref(file_item: dict) -> str | None:
                raw_url = str(file_item.get('url') or '').strip()
                if not raw_url:
                    return None
                if re.match(r'^(https?:|data:|blob:|/)', raw_url, re.IGNORECASE):
                    return raw_url
                if str(file_item.get('content_type') or '').startswith('image/'):
                    try:
                        return str(request.app.url_path_for('get_file_content_by_id', id=raw_url))
                    except Exception:
                        return raw_url
                return raw_url
                            ...
                            *[
                                {
                                    'type': 'image_url',
                                    'image_url': {'url': f['url']},
                                    'source_ref': _gallery_source_ref(f),
