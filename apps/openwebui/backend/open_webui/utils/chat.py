        ...
        if not selected_model_id and model.get('owned_by') == 'arena':
            ...
            if model_ids and filter_mode == 'exclude':
                model_ids = [
                    ...
                    if is_arena_text_candidate(available_model) and available_model['id'] not in model_ids
                ]
            ...
            if isinstance(model_ids, list) and model_ids:
                ...
                selected_model_id = random.choice(model_ids) if model_ids else None
            else:
                model_ids = [
                    available_model['id']
                    for available_model in list(request.app.state.MODELS.values())
            ...
            if selected_model_id:
                log.info(
                    "arena model resolved (chat fallback)",
                    extra={"selected_model_id": selected_model_id, "candidate_count": len(model_ids)},
                )
                form_data['model'] = selected_model_id
            else:
                log.warning("arena model resolution failed in chat fallback: no eligible chat models")
