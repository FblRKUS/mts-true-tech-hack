...
def _is_russian_language(request: Request) -> bool:
    header = (request.headers.get('accept-language') or '').lower()
    return 'ru' in header


def _autoselect_display_name(request: Request) -> str:
    return 'Авто' if _is_russian_language(request) else 'Auto'
...
async def get_all_models(request: Request, user: UserModel) -> dict[str, list]:
    ...
    auto_display_name = _autoselect_display_name(request)
            ...
            if model_list is not None and 'error' not in model_list:
                for model in model_list:
                    ...
                    if model_id and model_id not in models:
                        ...
                        if model_id in {'autoselect', 'auto'}:
