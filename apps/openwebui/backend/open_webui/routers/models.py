...
def get_model_profile_image(request: Request, id: str, user=Depends(get_verified_user)):
    def _resolve_external_avatar_url(raw_url: str) -> Optional[str]:
        ...
        # Prefer OpenWebUI proxy for GPTHub default avatar, so browser doesn't need direct GPTHub access.
        if candidate == '/v1/assets/model-avatars/default':
            return '/api/v1/gpthub/feature-settings/default-avatar'

        # Resolve GPTHub relative URLs against configured GPTHub API URL.
        if candidate.startswith('/'):
            gpthub_base = str(getattr(request.app.state.config, 'GPTHUB_API_URL', '') or '').strip()
            if gpthub_base:
                gpthub_base = gpthub_base.rstrip('/')
                if gpthub_base.endswith('/v1'):
                    gpthub_base = gpthub_base[:-3]
                return f'{gpthub_base}{candidate}'
    ...
    # Fallback for provider models (e.g. GPTHub/OpenAI list) that are not stored in Models table.
    external_models = getattr(request.app.state, 'OPENAI_MODELS', {}) or {}
    external_model = external_models.get(id)
    if isinstance(external_model, dict):
        avatar_candidate = (
            external_model.get('profile_image_url')
            or external_model.get('avatar_url')
            or (external_model.get('info', {}).get('meta', {}) or {}).get('profile_image_url')
        )
        resolved = _resolve_external_avatar_url(str(avatar_candidate or ''))
        if resolved:
            return Response(
                status_code=status.HTTP_302_FOUND,
