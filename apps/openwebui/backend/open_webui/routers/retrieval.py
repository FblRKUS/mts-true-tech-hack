...
def get_ef(
    ...
    if embedding_model and engine == '':
        try:
            ...
            log.warning(
                "sentence_transformers is not installed; local embeddings are disabled. "
                'Configure external embeddings (openai/ollama/azure_openai) or enable bypass mode.'
            )
...
def _resolve_openai_embedding_connection(request: Request) -> tuple[str, str, bool]:
    openai_base_url = request.app.state.config.RAG_OPENAI_API_BASE_URL or ''
    openai_api_key = request.app.state.config.RAG_OPENAI_API_KEY or ''

    openai_base_urls = request.app.state.config.OPENAI_API_BASE_URLS or []
    openai_api_keys = request.app.state.config.OPENAI_API_KEYS or []
    openai_api_configs = dict(request.app.state.config.OPENAI_API_CONFIGS or {})
    selected_openai_config = {}
    gpthub_api_base = str(request.app.state.config.GPTHUB_API_URL or '').strip().rstrip('/')
    openai_base_url_normalized = str(openai_base_url or '').strip().rstrip('/')
    prefer_openai_connection_list = bool(
        openai_base_urls
        and not openai_api_key
        and openai_base_url_normalized == 'https://api.openai.com/v1'
    )
    if gpthub_api_base.endswith('/v1'):
        gpthub_api_base = gpthub_api_base[:-3]

    if (not openai_base_url or prefer_openai_connection_list) and openai_base_urls:
        # Prefer configured GPTHub OpenAI connection when available.
        gpthub_idx = None
        for idx, url in enumerate(openai_base_urls):
            api_config = openai_api_configs.get(str(idx), openai_api_configs.get(url, {}))
            title = str(api_config.get('title', '')).strip().lower() if isinstance(api_config, dict) else ''
            if title == 'gpthub':
                gpthub_idx = idx
                break

        if gpthub_idx is None and gpthub_api_base:
            for idx, url in enumerate(openai_base_urls):
                normalized = str(url or '').strip().rstrip('/')
                if normalized in {gpthub_api_base, f'{gpthub_api_base}/v1'}:
                    gpthub_idx = idx
                    break

        if gpthub_idx is not None:
            openai_base_url = openai_base_urls[gpthub_idx]
            selected_openai_config = openai_api_configs.get(
                str(gpthub_idx),
                openai_api_configs.get(openai_base_url, {}),
            )
            if gpthub_idx < len(openai_api_keys):
                openai_api_key = openai_api_keys[gpthub_idx]
        else:
            openai_base_url = openai_base_urls[0]
            selected_openai_config = openai_api_configs.get(
                '0',
                openai_api_configs.get(openai_base_url, {}),
            )
    elif openai_base_url and openai_base_url in openai_base_urls:
        idx = openai_base_urls.index(openai_base_url)
        selected_openai_config = openai_api_configs.get(str(idx), openai_api_configs.get(openai_base_url, {}))

    if not openai_api_key:
        if openai_base_url and openai_base_url in openai_base_urls:
            idx = openai_base_urls.index(openai_base_url)
            if idx < len(openai_api_keys):
                openai_api_key = openai_api_keys[idx]
        elif openai_api_keys:
            openai_api_key = openai_api_keys[0]

    if not selected_openai_config and openai_base_url:
        selected_openai_config = openai_api_configs.get(openai_base_url, {})

    auth_type = (
        str(selected_openai_config.get('auth_type', '')).strip().lower()
        if isinstance(selected_openai_config, dict)
        else ''
    )
    allow_keyless_openai = bool(openai_base_url and auth_type == 'none')

    return openai_base_url, openai_api_key, allow_keyless_openai


def _resolve_openai_embedding_credentials(request: Request) -> tuple[str, str]:
    openai_base_url, openai_api_key, _ = _resolve_openai_embedding_connection(request)
    return openai_base_url, openai_api_key


def _has_openai_embedding_fallback(request: Request) -> bool:
    openai_base_url, openai_api_key, allow_keyless_openai = _resolve_openai_embedding_connection(request)
    return bool(openai_base_url and (openai_api_key or allow_keyless_openai))


EMBEDDINGS_NOT_CONFIGURED_WARNING = (
    'Embeddings are not configured. File text was saved, but vector indexing was skipped. '
    'Configure backend embeddings via RAG_OPENAI_API_BASE_URL/RAG_OPENAI_API_KEY, '
    'or set RAG_EMBEDDING_ENGINE to openai/ollama/azure_openai, or enable bypass mode.'
)


def _should_skip_vector_indexing(request: Request) -> tuple[bool, Optional[str]]:
    if request.app.state.config.BYPASS_EMBEDDING_AND_RETRIEVAL:
        return True, None

    if (
        request.app.state.config.RAG_EMBEDDING_ENGINE == ''
        and request.app.state.ef is None
        and not _has_openai_embedding_fallback(request)
    ):
        return True, EMBEDDINGS_NOT_CONFIGURED_WARNING
...
async def update_embedding_config(request: Request, form_data: EmbeddingModelUpdateForm, user=Depends(get_admin_user)):
    ...
    try:
        ...
        request.app.state.EMBEDDING_FUNCTION = get_embedding_function(
            ...
            allow_keyless_openai=allow_keyless_openai,
        )
...
async def update_rag_config(request: Request, form_data: ConfigForm, user=Depends(get_admin_user)):
    ...
    return {
        ...
        'web': {
            ...
            'WEB_SEARCH_RESULT_COUNT': request.app.state.config.WEB_SEARCH_RESULT_COUNT,
            'WEB_SEARCH_CONCURRENT_REQUESTS': request.app.state.config.WEB_SEARCH_CONCURRENT_REQUESTS,
            'FETCH_URL_MAX_CONTENT_LENGTH': getattr(
...
def process_file(
    ...
    if file:
        try:
            ...
            skip_indexing, warning_message = _should_skip_vector_indexing(request)
            if skip_indexing:
                update_data = {'status': 'completed'}
                if warning_message:
                    update_data['warning'] = warning_message
                    log.warning(f'Skipping vector indexing for file {file.id}: {warning_message}')
...
async def process_files_batch(
    ...
    if all_docs:
        ...
        if skip_indexing:
            if warning_message:
                log.warning(f'Skipping batch vector indexing for collection {collection_name}: {warning_message}')

            for file_result, (_, file_update) in zip(file_results, file_updates):
                update_data = {**(file_update.data or {}), 'status': 'completed'}
                if warning_message:
                    update_data['warning'] = warning_message

                Files.update_file_by_id(
                    id=file_result.file_id,
                    form_data=FileUpdateForm(hash=file_update.hash, data=update_data),
                )
