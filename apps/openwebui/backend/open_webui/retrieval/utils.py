...
async def query_collection(
    ...
    try:
        ...
        log.warning(f'Embedding lookup unavailable, falling back to full collection documents: {e}')
...
def get_embedding_function(
    ...
    allow_keyless_openai=False,
) -> Awaitable:
    if embedding_engine == '':
        if embedding_function is None:
            if url and (key or allow_keyless_openai):
                fallback_model = 'autoselect'

                async def openai_fallback_embedding_function(query, prefix=None, user=None):
                ...
                return openai_fallback_embedding_function

            async def embedding_not_configured(_query, prefix=None, user=None):
                raise RuntimeError(
                    "Local embeddings are not initialized. "
                    "Install 'sentence-transformers' and verify RAG_EMBEDDING_MODEL, "
                    "or configure RAG_OPENAI_API_BASE_URL/RAG_OPENAI_API_KEY for backend embeddings, "
                    "or set RAG_EMBEDDING_ENGINE to openai/ollama/azure_openai, "
                    "or enable BYPASS_EMBEDDING_AND_RETRIEVAL."
                )
...
async def get_sources_from_items(
    ...
    bypass_embedding_and_retrieval = bool(request and request.app.state.config.BYPASS_EMBEDDING_AND_RETRIEVAL)
    has_openai_embedding_fallback = False
    if request:
        ...
        allow_keyless_openai = auth_type == 'none'

        has_openai_embedding_fallback = bool(openai_base_url and (openai_api_key or allow_keyless_openai))

    embeddings_unavailable = bool(
        request
        and request.app.state.config.RAG_EMBEDDING_ENGINE == ''
        and request.app.state.ef is None
        and not has_openai_embedding_fallback
    )
    force_full_context = full_context or bypass_embedding_and_retrieval or embeddings_unavailable

    if embeddings_unavailable:
        log.warning('Embeddings are unavailable, using full-content retrieval for attached files and collections.')
    ...
    for item in items:
        ...
        if item.get('type') == 'text':
            ...
            if item.get('context') == 'full' or force_full_context:
