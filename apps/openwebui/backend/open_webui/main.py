    ...
    async def get_response(self, path: str, scope):
        try:
            ...
            if ex.status_code == 404:
                # Never mask missing API/backend routes with index.html
                # because frontend expects JSON status codes for these paths.
                if path.startswith(('api/', 'openai/', 'ollama/', 'ws/')):
                    raise ex
                if path.endswith('.js'):
...
app.state.config.GPTHUB_API_URL = GPTHUB_API_URL
...
openai_base_url = app.state.config.RAG_OPENAI_API_BASE_URL or ''
openai_api_key = app.state.config.RAG_OPENAI_API_KEY or ''
openai_base_urls = app.state.config.OPENAI_API_BASE_URLS or []
openai_api_keys = app.state.config.OPENAI_API_KEYS or []
openai_api_configs = dict(app.state.config.OPENAI_API_CONFIGS or {})
selected_openai_config = {}
gpthub_api_base = str(app.state.config.GPTHUB_API_URL or '').strip().rstrip('/')
openai_base_url_normalized = str(openai_base_url or '').strip().rstrip('/')
prefer_openai_connection_list = bool(
    openai_base_urls and not openai_api_key and openai_base_url_normalized == 'https://api.openai.com/v1'
)
...
allow_keyless_openai = (
    str(selected_openai_config.get('auth_type', '')).strip().lower() == 'none'
...
app.include_router(gpthub.router, prefix='/api/v1/gpthub', tags=['gpthub'])
...
async def download_generated_project(
    ...
    return await gpthub.download_generated_project(request=request, thread_id=thread_id, user=user)
